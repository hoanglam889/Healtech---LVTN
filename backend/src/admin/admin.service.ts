...
export class AdminService {
  ...
  async getDashboardStats() {
    const paidInvoices = await this.invoicesRepo.find({ where: { status: 'PAID' } });
  ...
  async getSchedules() {
    const list = await this.doctorSchedulesRepo.find({
      relations: { doctorProfile: { specialty: true }, shift: true },
      order: { date: 'ASC' },
    });
  ...
  async createSchedule(dto: { doctorProfileId: number; shiftId: any; date: string; maxPatients?: number }) {
    ...
    if (scheduleDate < today) {
      throw new BadRequestException({ i18nKey: 'errors.schedule.past_date' });
    }
    ...
    if (typeof shiftVal === 'string' || isNaN(+shiftVal)) {
      ...
      for (const shift of targetShifts) {
        const existing = await this.doctorSchedulesRepo.findOne({
          where: { doctorProfileId: dto.doctorProfileId, shiftId: shift.id, date: dto.date },
        });
        if (!existing) {
  ...
  async deleteSchedule(id: number) {
    ...
    if (!schedule) {
      throw new NotFoundException({ i18nKey: 'errors.schedule.not_found', args: { id } });
    }
    ...
    if (prefix) {
      const schedulesToDelete = await this.doctorSchedulesRepo.find({
        where: { doctorProfileId: schedule.doctorProfileId, date: schedule.date },
        relations: { shift: true },
      });
