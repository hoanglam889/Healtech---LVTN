...
const MyAppointments = ({ user, onBookClick }) => {
  ...
  const doCancel = () => {
    ...
    updateAppointment(id, { status: 'CANCELLED' })
      .then(() => { showToast(t('cancel_success'), 'success'); loadAppointments(); })
      .catch(() => showToast(t('cancel_error'), 'error'));
  };
  ...
  const filteredAppointments = appointments.filter((apt) => {
    ...
    if (search && !doctorName.includes(search)) return false;
    return true;
  });

  const filterTabs = [
    { label: t('common:all', 'All'), value: 'ALL' },
    { label: t('filter_upcoming'), value: 'BOOKED' },
    { label: t('filter_done'), value: 'DONE' },
    { label: t('filter_cancelled'), value: 'CANCELLED' },
  ];
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
          <div className="space-y-1">
            <h1 className="text-2xl font-bold text-gray-900 md:text-3xl flex items-center gap-2">
              ...
              <span>{t('my_appointments_title')}</span>
            </h1>
            <p className="text-sm text-gray-400 font-semibold">{t('my_appointments_subtitle')}</p>
          </div>
          <button
            onClick={onBookClick}
